...
class OccDuke(BaseDataset):
    def __init__(self,
        ...
        super(OccDuke, self).__init__(ignore_label, base_size,
        ...
        self.train_dir = os.path.join(self.root, 'Occluded_Duke', 'bounding_box_train')
        self.query_dir = os.path.join(self.root, 'Occluded_Duke', 'query')
        self.gallery_dir = os.path.join(self.root, 'Occluded_Duke', 'bounding_box_test')

        self.files = self._process_dir(self.train_dir)
        # self.files = self._process_dir(self.query_dir)
        # self.files = self._process_dir(self.gallery_dir)
    ...
    def gen_sample_without_label(self, image, multi_scale=True,
        ...
        image = self.input_transform(image) # 归一化，(384, 128, 3)
        image = image.transpose((2, 0, 1))  # (3, 384, 128)
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(img_path, cv2.IMREAD_COLOR)  #(285, 113, 3)
        size = image.shape[:-1] # (285, 113)

        if self.flip:   # False
            flip = np.random.choice(2) * 2 - 1
            image = image[:, ::flip, :]

        image_size = (self.crop_size[1], self.crop_size[0]) #tuple(128, 384)
    ...
    def multi_scale_inference(self, model, image, scales=False, flip=False):
        size = image.size()
        pred = model(image) # torch.Size([1, 10, 96, 32])
        pred = F.upsample(input=pred,   # 使用F.upsample函数对预测结果进行上采样，使其大小与原始图像相同
                          size=(size[-2], size[-1]),
                          mode='bilinear')
        if flip:
