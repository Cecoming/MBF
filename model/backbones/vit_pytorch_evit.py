...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_rate=1., fuse_token=False):
        super().__init__()
    ...
    def forward(self, x, keep_rate=None, get_idx=False):
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu =1.0, keep_rate=(1, ), fuse_token=False):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                keep_rate=keep_rate[i], fuse_token=fuse_token)  # added
    ...
    def forward_features(self, x, camera_id, view_id, keep_rate=None, get_idx=False):
        B, _, h, w = x.shape
    ...
    #     return x
    def forward(self, x, cam_label=None, view_label=None, keep_rate=None,  get_idx=False):
        if keep_rate == None:
            keep_rate = self.keep_rate
        x, _, idxs = self.forward_features(x, cam_label, view_label, keep_rate, get_idx)
        # x = self.head(x)  #??
...
def vit_base_patch16_224_ETransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5,base_keep_rate=0.8, drop_loc=(3, 6, 9), **kwargs):
    ...
    model = TransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, keep_rate=keep_rate, **kwargs)
