...
def estimate_tsen(datas,labs,dim,target_perplexity,plot=False,path=None):
    ...
    for m_iter in range(1, max_iter+1):
        ...
        if plot and m_iter % max_iter == 0:
            if path is not None:
                fname = path + "/" + str(m_iter) + "-001.jpg"
            else:
                fname = str(m_iter) + "-001.jpg"
            print("Draw Map")
            draw_pic(Y,labs,name = fname)
...
def draw_pic(datas,labs,name = '1.jpg'):
    ...
    for i in range(len(unque_labs)):
        ...
        # Add a scatter plot for the "outer ring" with larger size and different color
        plt.scatter(datas[index, 0], datas[index, 1], s=60, facecolors='none', edgecolors='black')
...
if __name__ == "__main__":
    ...
    mnist_datas = np.loadtxt("/home/ssy/occluded/python-reduce/mnist2500_X.txt")    # (2500, 784)
    mnist_labs = np.loadtxt("/home/ssy/occluded/python-reduce/mnist2500_labels.txt")    # (2500,)
